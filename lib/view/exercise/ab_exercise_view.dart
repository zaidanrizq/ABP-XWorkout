...
class AbExerciseView extends StatefulWidget {
  final Map eObj;
  const AbExerciseView({super.key, required this.eObj});

  @override
  State<AbExerciseView> createState() => _AbExerciseViewState();
}

class _AbExerciseViewState extends State<AbExerciseView> {
  final _firestore = FirebaseFirestore.instance;

  TextEditingController txtSearch = TextEditingController();

  List<Map<String, dynamic>> absMovesList = [];

  void getAbMoves() async {
    ...
    absMovesList = moves.docs.map((move) {
      return {
        'targetMuscleId': 'Abs',
        'moveId': move.id,
        'name': move.data()['name'],
        ...
        child: Column(
          ...
          children: [
            ...
            ListView.builder(
                ...
                shrinkWrap: true,
                itemCount: absMovesList.length,
                itemBuilder: (context, index) {
                  var fObj = absMovesList[index] as Map? ?? {};
                  return InkWell(
                    onTap: () {
                      Navigator.push(
                        context,
